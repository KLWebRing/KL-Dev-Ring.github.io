// ── KL DevVerse — Developer Town District ───────────────────────────
// The heart of KL DevVerse. Central plaza, town hall, roads, river,
// bridge, mountains, trees, streetlights, member houses, hall of fame.
//
// This is the original WorldScene.tsx geometry, extracted verbatim
// into a district builder function. Zero visual regression.
...
import { districtRegistry, type DistrictBuildContext, type DistrictBuildResult, type NPCEntry } from '../DistrictRegistry';
import type { InteractableItem, Structure, AnimationState } from '@/shared/types';

// ── Builder ─────────────────────────────────────────────────────────

function buildDeveloperTown(ctx: DistrictBuildContext): DistrictBuildResult {
  const townGroup = new Group();
  const structures: Structure[] = [];
  const interactables: InteractableItem[] = [];
  const npcs: NPCEntry[] = [];

  const onInteract = ctx.onInteract;
  ...
  // ── BACKWATER RIVER ─────────────────────────────────────────────
  const riverTex = createRiverTexture();
  const riverMat = toon(0x0284c7, { map: riverTex, transparent: true, opacity: 0.85 });
  ...
  // ── BUILDER STUDIOS (Neighborhood) ─────────────────────────────
  ctx.members.forEach((member, i) => {
    const isLeft = i % 2 === 0;
    const row = Math.floor(i / 2);
    const plotX = isLeft ? 15 : -15;
    const plotZ = 28 + row * 22;

    // Studio exterior placeholder
    const studioGroup = new Group();
    studioGroup.position.set(plotX, 0, plotZ);
    ...
    studioGroup.add(roof);

    townGroup.add(studioGroup);
    structures.push({ x: plotX, z: plotZ, radius: 3.2 });

    interactables.push({
      x: plotX, z: plotZ + 3.5, radius: 1.8,
      message: `Press [E] to enter ${member.name}'s Studio`,
      action: () => onInteract({ type: 'passport', member }),
    });

    // Resident NPC outside the studio
  ...
  ctx.members.slice(0, 3).forEach((member, i) => {
  ...
  return {
    ...
    interactables,
    npcs,
    riverTexture: riverTex,
  };
}

// ── Register ────────────────────────────────────────────────────────

districtRegistry.register('developer_town', buildDeveloperTown);
